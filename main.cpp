...
int main() {
  ...
  var lst = default list,
    ...
    lst.push_back(*curr_msg_ptr->value),
        remote mutable = curr_msg_ptr, curr_msg_ptr = curr_msg_ptr->next,
               mutable.next = mutable.next.nulled(),
               var sample_user_hndl = (g->value).users->value.ensure(causal),
               var x = 3, var y = 5, x = 7, y = y + x, var condition = 0,
               while (condition < x){y = y + 3,
    ...
    , return ret, var iterator = users, interator.advance(),
                      ...
                      if (true || false) {
      return b1
    } else {return b2}, while (iterator.isValid()) {iterator = iterator->next}, return b1); //*/
    ...
    constexpr auto second_allocator = as_types::as_value<1000,typename
    flatten<wrapper>::parse_t>(); pretty_print(std::cout, second_allocator.top,
    second_allocator); std::cout << std::endl << std::endl; print(std::cout,
    second_allocator.top, second_allocator); //*/
