...
template <typename prev_holder> struct as_type_f {
  ...
  as_type(std::enable_if_t<(budget > 0) && (budget <= 10000)> * = nullptr) {
...
template <typename prev_holder> constexpr auto as_type() {
  ...
  return as_types::Statement<as_types::transaction<
      DECT(as_type_f<prev_holder>::template as_type<1000, arg>()),
...
print(std::ostream &o,
      const plain_array<
          allocated_ref<myria::mtl::new_parse_phase::as_values::AST_elem>> &arr,
      const Allocator &a) {
  o << "{";
  for (auto &ref : arr) {
    if (ref) {
