...
template <typename string> struct parse {
  ...
  parse_fieldref(const str_t &str) {
    ...
    last_split('.', trimmed, operands);
    {
      ...
      if (contains_paren(operands[1]))
        throw parse_error{"Parse error: We thought this was a fieldref, but it "
                          "contains parens"};
      if (contains_space(operands[1]))
        throw parse_error{"Parse error: We thought this was a fieldref, but it "
                          "contains whitespace"};
      if (contains_outside_parens(".", operands[1]))
        throw parse_error{"Parse error: This should be a fieldref, but it "
  ...
  constexpr allocated_ref<as_values::AST_elem>
  parse_fieldptrref(const str_t &str, const char *) {
    using namespace mutils;
    using namespace cstring;
    str_nc trimmed = {0};
    str_nc operands[2] = {{0}};
    ...
    last_split("->", trimmed, operands);
    {
      // error checking
      if (contains_paren(operands[1]))
        throw parse_error{"Parse error: We thought this was a fieldptrref, but "
                          "it contains parens"};
      if (contains_space(operands[1]))
        throw parse_error{"Parse error: We thought this was a fieldptrref, but "
                          "it contains whitespace"};
      if (contains_outside_parens(".", operands[1]))
        throw parse_error{"Parse error: This should be a fieldptrref, but it "
                          "contains a '.', which is not allowed"};
      if (contains_outside_parens("->", operands[1]))
        throw parse_error{"Parse error: This should be a fieldptrref, but it "
                          "contains a '->', which is not allowed"};
  ...
  constexpr allocated_ref<as_values::AST_elem> parse_varref(const str_t &str) {
    ...
    trim(ref.Var, str);
    {
      ...
      if (contains_paren(ref.Var))
        throw parse_error{"Parse error: We thought this was a variable access, "
                          "but it contains parens"};
      if (contains_space(ref.Var))
        throw parse_error{"Parse error: We thought this was a variable access, "
                          "but it contains whitespace"};
      if (contains_outside_parens(".", ref.Var))
        throw parse_error{"Parse error: This should be a variable access, but "
                          "it contains a '.', which is not allowed"};
      if (contains_outside_parens("->", ref.Var))
        throw parse_error{"Parse error: This should be a variable access, but "
                          "it contains a '->', which is not allowed"};
      if (contains_outside_parens("*", ref.Var))
  ...
  constexpr allocated_ref<as_values::AST_elem>
  parse_expression(const str_t &_str) {
    using namespace mutils;
    using namespace cstring;
    str_nc str = {0};
    trim(str, _str);
  ...
  constexpr allocated_ref<as_values::AST_elem> parse_remote(const str_t &str) {
    ...
    allocated_ref<as_values::AST_elem> ret =
        allocator.template allocate<as_values::AST_elem>();
    ret.get(allocator).template get_<as_values::LetRemote>().is_this_elem =
        true;
  ...
  constexpr parse() {
    ...
    if (contains(';', local_copy))
      throw parse_error{
          "Parse Error: Semicolons have no place here.  Did you mean ','? "};
