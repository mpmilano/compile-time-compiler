  ...
  constexpr allocated_ref<as_values::AST_elem> parse_sequence(const str_t& str)
  {
    ...
    for (auto i = 0u; i < groups; ++i) {
      ...
      if ((i + 1) < groups) {
        ...
        allocated_ref<as_values::AST_elem> newret = allocator.template allocate<as_values::AST_elem>();
        newret.get(allocator).template get_<as_values::Sequence>().is_this_elem = true;
        auto& seqref = newret.get(allocator).template get_<as_values::Sequence>().t;
        seq->next = std::move(newret);
        seq = &seqref;
      } else {

        allocated_ref<as_values::AST_elem> skipref = allocator.template allocate<as_values::AST_elem>();
        skipref.get(allocator).template get_<as_values::Skip>().is_this_elem = true;
